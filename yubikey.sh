...
cat /etc/pam.d/sudo-i | if ! grep -q pam_u2f.so; then echo -e "sudo ${RED}failed${ENDCOLOR}"; else echo "sudo ${GREEN}passed${ENDCOLOR}"; fi
cat /etc/pam.d/sudo-i | if ! grep -q pam_u2f.so; then echo -e "gdm-password ${RED}failed${ENDCOLOR}"; else echo "gdm-password ${GREEN}passed${ENDCOLOR}"; fi
cat /etc/pam.d/sudo-i | if ! grep -q pam_u2f.so; then echo -e "login ${RED}failed${ENDCOLOR}"; else echo "login ${GREEN}passed${ENDCOLOR}"; fi
cat /etc/pam.d/sudo-i | if ! grep -q pam_u2f.so; then echo -e "su ${RED}failed${ENDCOLOR}"; else echo "su ${GREEN}passed${ENDCOLOR}"; fi
